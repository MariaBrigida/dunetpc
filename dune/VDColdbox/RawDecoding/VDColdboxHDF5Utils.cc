...
void closeFile(HDFFileInfoPtr hdfFileInfoPtr) {
  ...
  {
    hid_t grp = H5Gopen(hdfFileInfoPtr->filePtr, "/", H5P_DEFAULT);
    std::vector<std::string> myList = readMidLevelGroupNames(grp);
    H5Gclose(grp);
    return myList;
  }
  ...
  void getFragmentsForEvent(hid_t the_group, RawDigits& raw_digits, RDTimeStamps &timestamps, int apano) 
  {
    ...
    std::vector<std::string> det_types
      = dune::VDColdboxHDF5Utils::readMidLevelGroupNames(the_group);
	...
	// apaNames is a vector whose elements start at [0].
        hid_t linkGroup = dune::VDColdboxHDF5Utils::getGroupFromPath(geoGroup, apaNames[apano-1]);
	std::vector<std::string> linkNames
          = dune::VDColdboxHDF5Utils::readMidLevelGroupNames(linkGroup);
